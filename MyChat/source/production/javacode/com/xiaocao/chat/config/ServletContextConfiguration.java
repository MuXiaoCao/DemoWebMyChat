 ...
 * 1. 配置消息转换器
 * 2. 配置内容协商
 * 3. 在controller中使用@ResponseBody,返回对象
 * 4. 在返回的类中使用对应注解
 * 作用：通过消息转换器可以将响应的数据转换为控制器方法可以处理的某种类型的java对象
...
public class ServletContextConfiguration extends WebMvcConfigurerAdapter{
	...
	@Inject Marshaller marshaller;    // 后两个其实是一个
	...
	public void configureContentNegotiation(ContentNegotiationConfigurer configurer) {
		...
		// 禁用了jaf（java activation framework），它可以将文件扩展名映射到媒体类型的工具。我们将采用手动指定可用的媒体类型
	...
	public ViewResolver viewResolver() {
		...
		resolver.setPrefix("WEB-INF/html/view/");
